...
#include "morphagene-splice.h"
#include "morphagene-grain.h"
#include <cmath>

/*
 * Morphagene DSP Processor
 *
 * Main DSP processing engine that combines all Morphagene components:
...
public:
  ...
  void setSos(float sos) noexcept { sosParam_ = MorphageneUtil::clamp01(sos); }
  void setGeneSize(float size) noexcept { geneSizeParam_ = MorphageneUtil::clamp01(size); }
  void setMorph(float morph) noexcept { morphParam_ = MorphageneUtil::clamp01(morph); }
  void setSlide(float slide) noexcept { slideParam_ = MorphageneUtil::clamp01(slide); }
  void setOrganize(float organize) noexcept
  {
    organizeParam_ = MorphageneUtil::clamp01(organize);
  ...
  {
    variSpeedParam_ = MorphageneUtil::clamp01(speed);
  }
  ...
  {
    geneSizeCv_ = cv;
    geneSizeCvAtten_ = MorphageneUtil::clamp(atten, -1.0f, 1.0f);
  }

  void setVariSpeedCv(float cv, float atten) noexcept
  {
    variSpeedCv_ = cv;
    variSpeedCvAtten_ = MorphageneUtil::clamp(atten, -1.0f, 1.0f);
  }

  void setSlideCv(float cv, float atten) noexcept
  {
    slideCv_ = cv;
    slideCvAtten_ = MorphageneUtil::clamp(atten, -1.0f, 1.0f);
  }
  ...
  void stopAutoLevel() noexcept
  {
    ...
    if (autoLevelPeak_ > 0.001f)
    {
      ...
      autoLevelGain_ = MorphageneUtil::clamp(autoLevelGain_, 0.1f, 10.0f);
    }
  }
  ...
  ProcessResult process(float audioInL, float audioInR) noexcept
  {
    ...
    if (sosCv_ != 0.0f)
    {
      effectiveSos += sosCv_ / MorphageneConfig::kSosCvMax;
      effectiveSos = MorphageneUtil::clamp01(effectiveSos);
    }

    float effectiveGeneSize = geneSizeParam_;
    effectiveGeneSize += (geneSizeCv_ / MorphageneConfig::kGeneSizeCvMax) * geneSizeCvAtten_;
    effectiveGeneSize = MorphageneUtil::clamp01(effectiveGeneSize);

    float effectiveMorph = morphParam_;
    if (morphCv_ != 0.0f)
    {
      effectiveMorph += morphCv_ / MorphageneConfig::kMorphCvMax;
      effectiveMorph = MorphageneUtil::clamp01(effectiveMorph);
    }

    float effectiveSlide = slideParam_;
    effectiveSlide += (slideCv_ / MorphageneConfig::kSlideCvMax) * slideCvAtten_;
    effectiveSlide = MorphageneUtil::clamp01(effectiveSlide);

    float effectiveOrganize = organizeParam_;
    if (organizeCv_ != 0.0f)
    {
      effectiveOrganize += organizeCv_ / MorphageneConfig::kOrganizeCvMax;
      effectiveOrganize = MorphageneUtil::clamp01(effectiveOrganize);
    ...
    variSpeedState_ = MorphageneUtil::calculateVariSpeed(
        variSpeedBipolar, variSpeedCv_, variSpeedCvAtten_);

    // Update morph state
    morphState_ = MorphageneUtil::calculateMorphState(effectiveMorph);
    ...
    float geneSizeSamples = MorphageneUtil::calculateGeneSizeSamples(
    ...
    result.cvOut = envelopeValue_ * MorphageneConfig::kCvOutMax;
  ...
  const MorphageneBuffer &getBuffer() const noexcept { return buffer_; }
  MorphageneBuffer &getBuffer() noexcept { return buffer_; }
                ...
                const std::vector<size_t> &markers = {}) noexcept
  {
    ...
    size_t framesToLoad = std::min(numFrames, MorphageneBuffer::kMaxFrames);
...
private:
                        ...
                        float sosAmount) noexcept
  {
    ...
    if (recordState_.recordPosition >= MorphageneBuffer::kMaxFrames)
    {
      stopRecording();
      return;
    }
