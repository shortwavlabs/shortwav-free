...
class MorphageneBuffer
{
public:
  static constexpr size_t kMaxFrames = MorphageneConfig::kMaxReelFrames;
  ...
  void readStereoInterpolated(double position, float &outL, float &outR) const noexcept
  {
    ...
    // Left channel
    outL = MorphageneUtil::cubicInterpolate(
        data_[i0 * kChannels],
        data_[i1 * kChannels],
        data_[i2 * kChannels],
        data_[i3 * kChannels],
    ...
    // Right channel
    outR = MorphageneUtil::cubicInterpolate(
