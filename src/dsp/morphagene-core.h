...
/*
 * Morphagene Core Types and Configuration
 *
 * Core data structures and configuration constants for the Morphagene
...
{
  const float minGene = MorphageneConfig::kMinGeneSamples;
...
inline VariSpeedState calculateVariSpeed(float param, float cvInput, float cvAtten) noexcept
{
  ...
  float combined = param + (cvInput / MorphageneConfig::kVariSpeedCvMax) * cvAtten;
  ...
  if (state.isForward)
  {
    semitones = absParam * MorphageneConfig::kVariSpeedUpSemitones;
  }
  else
  {
    semitones = absParam * MorphageneConfig::kVariSpeedDownSemitones;
  }
