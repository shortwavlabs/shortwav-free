...
void Morphagene::process(const ProcessArgs& args)
{
  ...
  // Process DSP
  ShortwavDSP::MorphageneDSP::ProcessResult result = dsp.process(audioInL, audioInR);
...
void Morphagene::processButtons(const ProcessArgs& args)
{
...
void Morphagene::processButtonCombos(const ProcessArgs& args)
{
...
void Morphagene::processGateInputs(const ProcessArgs& args)
{
  // CLK input
  if (inputs[CLK_INPUT].isConnected())
  {
    if (clkTrigger.process(inputs[CLK_INPUT].getVoltage(),
                           0.1f, ShortwavDSP::MorphageneConfig::kGateTriggerThreshold))
    {
      dsp.onClockRising();
    }
  }
  ...
  {
    bool playHigh = inputs[PLAY_INPUT].getVoltage() >= ShortwavDSP::MorphageneConfig::kGateTriggerThreshold;
...
void Morphagene::updateLights(const ProcessArgs& args)
{
  ...
  float envLevel = dsp.getEnvelopeValue() / ShortwavDSP::MorphageneConfig::kCvOutMax;
...
void Morphagene::loadFileAsync(const std::string& path)
{
...
void Morphagene::saveFileAsync(const std::string& path)
{
  if (fileLoading.load() || fileSaving.load())
    return;
...
void Morphagene::dataFromJson(json_t* rootJ)
{
  // Load reel index
...
MorphageneWidget::MorphageneWidget(Morphagene* module)
{
  setModule(module);
  this->module = module;

  // 20HP panel
  setPanel(APP->window->loadSvg(asset::plugin(pluginInstance, "res/Morphagene.svg")));
  ...
  float yPos = 95;
  addInput(createInputCentered<PJ301MPort>(Vec(25, yPos), module, Morphagene::AUDIO_IN_L));
  addInput(createInputCentered<PJ301MPort>(Vec(55, yPos), module, Morphagene::AUDIO_IN_R));

  // Audio outputs (top right)
  addOutput(createOutputCentered<PJ301MPort>(Vec(box.size.x - 55, yPos), module, Morphagene::AUDIO_OUT_L));
  addOutput(createOutputCentered<PJ301MPort>(Vec(box.size.x - 25, yPos), module, Morphagene::AUDIO_OUT_R));

  // S.O.S. knob and CV
  yPos = 135;
  addParam(createParamCentered<RoundBlackKnob>(Vec(35, yPos), module, Morphagene::SOS_PARAM));
  addInput(createInputCentered<PJ301MPort>(Vec(35, yPos + 35), module, Morphagene::SOS_CV_INPUT));

  // Gene Size knob, attenuverter, and CV
  addParam(createParamCentered<RoundBlackKnob>(Vec(95, yPos), module, Morphagene::GENE_SIZE_PARAM));
  addParam(createParamCentered<Trimpot>(Vec(95, yPos + 28), module, Morphagene::GENE_SIZE_CV_ATTEN));
  addInput(createInputCentered<PJ301MPort>(Vec(95, yPos + 55), module, Morphagene::GENE_SIZE_CV_INPUT));

  // Morph knob and CV
  addParam(createParamCentered<RoundBlackKnob>(Vec(155, yPos), module, Morphagene::MORPH_PARAM));
  addInput(createInputCentered<PJ301MPort>(Vec(155, yPos + 35), module, Morphagene::MORPH_CV_INPUT));

  // Slide knob, attenuverter, and CV
  addParam(createParamCentered<RoundBlackKnob>(Vec(215, yPos), module, Morphagene::SLIDE_PARAM));
  addParam(createParamCentered<Trimpot>(Vec(215, yPos + 28), module, Morphagene::SLIDE_CV_ATTEN));
  addInput(createInputCentered<PJ301MPort>(Vec(215, yPos + 55), module, Morphagene::SLIDE_CV_INPUT));
  ...
  addChild(createLightCentered<MediumLight<RedGreenBlueLight>>(
      Vec(55, yPos), module, Morphagene::VARI_SPEED_LEFT_LIGHT));
  addChild(createLightCentered<MediumLight<RedGreenBlueLight>>(
      Vec(box.size.x - 55, yPos), module, Morphagene::VARI_SPEED_RIGHT_LIGHT));

  // Vari-Speed knob (center)
  addParam(createParamCentered<RoundLargeBlackKnob>(Vec(box.size.x / 2, yPos), module, Morphagene::VARI_SPEED_PARAM));

  // Vari-Speed CV
  yPos = 270;
  addParam(createParamCentered<Trimpot>(Vec(box.size.x / 2 - 30, yPos), module, Morphagene::VARI_SPEED_CV_ATTEN));
  addInput(createInputCentered<PJ301MPort>(Vec(box.size.x / 2 + 30, yPos), module, Morphagene::VARI_SPEED_CV_INPUT));

  // Organize knob and CV
  addParam(createParamCentered<RoundBlackKnob>(Vec(box.size.x - 45, yPos), module, Morphagene::ORGANIZE_PARAM));
  addInput(createInputCentered<PJ301MPort>(Vec(box.size.x - 45, yPos + 35), module, Morphagene::ORGANIZE_CV_INPUT));

  // Activity LEDs row
  yPos = 305;
  addChild(createLightCentered<MediumLight<RedGreenBlueLight>>(Vec(40, yPos), module, Morphagene::REEL_LIGHT));
  addChild(createLightCentered<MediumLight<RedGreenBlueLight>>(Vec(100, yPos), module, Morphagene::SPLICE_LIGHT));
  addChild(createLightCentered<MediumLight<RedGreenBlueLight>>(Vec(160, yPos), module, Morphagene::CV_OUT_LIGHT));

  // Gate inputs row
  yPos = 335;
  addInput(createInputCentered<PJ301MPort>(Vec(25, yPos), module, Morphagene::CLK_INPUT));
  addInput(createInputCentered<PJ301MPort>(Vec(60, yPos), module, Morphagene::PLAY_INPUT));
  addInput(createInputCentered<PJ301MPort>(Vec(95, yPos), module, Morphagene::REC_INPUT));
  addInput(createInputCentered<PJ301MPort>(Vec(130, yPos), module, Morphagene::SPLICE_INPUT));
  addInput(createInputCentered<PJ301MPort>(Vec(165, yPos), module, Morphagene::SHIFT_INPUT));

  // CV and EOSG outputs
  addOutput(createOutputCentered<PJ301MPort>(Vec(box.size.x - 60, yPos), module, Morphagene::EOSG_OUTPUT));
  addOutput(createOutputCentered<PJ301MPort>(Vec(box.size.x - 25, yPos), module, Morphagene::CV_OUTPUT));

  // Buttons with LEDs
  yPos = 365;
  addParam(createParamCentered<LEDButton>(Vec(95, yPos), module, Morphagene::REC_BUTTON));
  addChild(createLightCentered<MediumLight<RedLight>>(Vec(95, yPos), module, Morphagene::REC_LED));

  addParam(createParamCentered<LEDButton>(Vec(130, yPos), module, Morphagene::SPLICE_BUTTON));
  addChild(createLightCentered<MediumLight<YellowLight>>(Vec(130, yPos), module, Morphagene::SPLICE_LED));

  addParam(createParamCentered<LEDButton>(Vec(165, yPos), module, Morphagene::SHIFT_BUTTON));
  addChild(createLightCentered<MediumLight<GreenLight>>(Vec(165, yPos), module, Morphagene::SHIFT_LED));
}
...
Model* modelMorphagene = createModel<Morphagene, MorphageneWidget>("Morphagene");
